...
namespace VideoStoreApi.Utils
{
    public class EmployeeUtils
    {
        ...
        public bool CreateNewUser(TempEmployee temp)
        {
            try
            {
                if (PasswordUtils.IsPasswordFormatValid(temp.RawPw))
                {
                    ...
                    string createUserQuery = "INSERT INTO forclass.employeelist(EMP_Name_First, EMP_Name_Last, EMP_PW_Hash,EMP_Active, EMP_Type) " +
                    ...
                    createUser.makeDBQuery(createUserQuery,false);
                    return true;
                }
                else
                {
        ...
        {
            string loginStringQuery = $"SELECT * FROM forclass.employeelist WHERE EMP_ID = {empId};";
        ...
        public bool EditEmployeeAccount(TempEmployee updatedEmployee)
        {
            try
            {
                if (PasswordUtils.IsPasswordFormatValid(updatedEmployee.RawPw))
                {
                    ...
                    string updateEmployeeInfoQuery = "UPDATE forclass.employeelist " +
                    ...
                    updateEmployee.makeDBQuery(updateEmployeeInfoQuery,false);
                    return true;
                }
                else
                {
        ...
        private Permission GetEmployeePermissions(int PermissionLvl)
        {
            try
            {
                string sqlQueryString = "SELECT * " +
                                        "FROM forclass.employeepermissions " +
